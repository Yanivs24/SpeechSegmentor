...
parser.add_argument('--experiment_suffix', help='suffix for the experiment folder name', default="")
args = parser.parse_args()
dargs = vars(args)

# create folder for the new experiment
experiments_folder = 'experiments'
experiment_name = "{}_{}_{}_{}".format(args.dataset, time.strftime("%H_%M_%S"), time.strftime("%d_%m_%Y"), args.experiment_suffix)
...
# create grid
grids = {'learning_rate': [0.001],
        'reg_loss_freq': [-1, 2, 3, 10],
         # 'use_k': [True],
...
for sub_exp_idx, combination in enumerate(cartesian_product):
    ...
    dargs['output_file'] = sub_exp_name + ".log"
